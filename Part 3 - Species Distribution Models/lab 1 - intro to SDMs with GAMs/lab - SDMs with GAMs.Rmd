...
Once we've fit a model with our chosen combination of predictors, we can plot the smoothers from these models by just using the "plot" function. We can plot spatial, temporal, and spatio-temporal smoothers in the same way.
...
We can also use the ROC curve to obtain a good threshold probability for deciding whether we should classify a predicted probability of occurrence as "present" or "absent." The threshold that maximizes the sum of the sensitivity (the true positive rate) and specificity (the true negative rate) can be obtained with: 

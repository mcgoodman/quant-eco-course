...
Where $\phi^{-1}$ is the inverse cumulative distribution function of the standard Normal distribution (a normal distribution with $\mu = 0$ and $\sigma = 1$). Equivalently:
...
Now, play around with simulating new data - can you recover the parameters that you simulated the data from? What happens to the estimates and their SE when you decrease the sample size of the simulated data? Do the Fisher information and bootstrap methods agree on small datasets? On large ones? **2 pt.**
